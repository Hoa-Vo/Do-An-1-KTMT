...
istream& ScanQInt(istream& inDev, QInt& x, int type, string str) {
	isOutRange = false;
	if (type == 2) {
...
QInt& operator*(QInt x, QInt y) {
	...
	while (k > 0) {
		if (getBit(y.data[3], 0) && q == 0) {
			temp = temp - x;
		}
		else if (getBit(y.data[3], 0) == 0 && q) {
			temp = temp + x;
		}
		shiftRight(temp, y, q, result);
		k--;
	}
...
void combineArray(QInt temp, QInt q, bool q1, bool result[]) {
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 31; j >= 0; j--) {
			result[index++] = getBit(temp.data[i], j);
		}
	}
	for (int i = 0; i < 4; i++) {
		for (int j = 31; j >= 0; j--) {
			result[index++] = getBit(q.data[i], j);
		}
	}
	result[256] = q1;
}
void splitArray(bool result[], QInt& temp, QInt& q, bool& q1) {
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 31; j >= 0; j--) {
			if (result[index] && getBit(temp.data[i], j) == 0) {
				temp.data[i] = setBit1(temp.data[i], j);
			}
			else if (result[index] == 0 && getBit(temp.data[i], j))
				temp.data[i] = setBit0(temp.data[i], j);
			index++;
		}
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 31; j >= 0; j--) {
			if (result[index] && getBit(q.data[i], j) == 0) {
				q.data[i] = setBit1(q.data[i], j);
			}
			else if (result[index] == 0 && getBit(q.data[i], j))
				q.data[i] = setBit0(q.data[i], j);
			index++;
		}
	}
}
